...
class ModelStudyScreen extends StatefulWidget {
  const ModelStudyScreen({Key? key}) : super(key: key);

  @override
  State<ModelStudyScreen> createState() => _ModelStudyScreenState();
}

class _ModelStudyScreenState extends State<ModelStudyScreen> {
  late Database db;
  bool isLoading = true;
  ...
  reCheckExams() async {
    ...
    if (subId == null) {
      ...
      maps = await db.query('exam', where: "subject=${subId} AND (name LIKE 'Model%')");
    }
    print(maps);
    maps = List.from(maps.reversed);
