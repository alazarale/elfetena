...
class _WrongAnswerState extends State<WrongAnswer> {
  ...
  List<int> _unansweredQuestionIds = []; // Store unanswered question IDs as integers
  List<int> _allAnsweredQuestionIds = []; // Store all question IDs from the result
  ...
  Map<String, double> dataMap = {'none': 3, 'none2': 5}; // Keep if needed for other analysis
  Map<String, String> chapterMap = {}; // Map chapter ID (string) to "Unit: Name"
  Map<int, Map<String, dynamic>> questionMap = {}; // Store questions by ID
  List<Map<String, dynamic>> _wrongQuestions = []; // List to hold only the wrong questions
  ...
  getResults(resultId) async {
    ...
    _rightQuestionIds = _results['right'].toString().split(',').where((id) => id.isNotEmpty).map((id) => int.parse(id)).toList();
    _wrongQuestionIds = _results['wrong'].toString().split(',').where((id) => id.isNotEmpty).map((id) => int.parse(id)).toList();
    _unansweredQuestionIds = _results['unanswered'].toString().split(',').where((id) => id.isNotEmpty).map((id) => int.parse(id)).toList();
    _allAnsweredQuestionIds = [..._rightQuestionIds, ..._wrongQuestionIds, ..._unansweredQuestionIds];
    ...
    // Filter questions to only include those that were answered wrongly in the current result
    _wrongQuestions = ques.where((question) => _wrongQuestionIds.contains(question['id'])).toList();
    ...
    subchapters.forEach((element) {
      subchapterMap[element['id']] = {'name': element['name'], 'chapter': element['chapter']};
    });


    setState(() {
      isLoading = false; // Set loading to false after data is fetched and processed
    });
  }

  // Helper function to build choice text with highlighting
  Widget _buildChoiceText(String choice, String? text, String correctAnswer, String? userChosenAnswer) {
    ...
    if (userChosenAnswer != null && choice == userChosenAnswer && choice != correctAnswer) {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 10),
                // Display total number of wrong questions
                Center(
                  child: Text(
                    'Total Wrong Questions: ${_wrongQuestions.length}',
                    style: TextStyle(
                ...
                // Use ListView.builder to display wrong questions
                ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final String? userChosenAnswer = userWrongAnswers[questionId]; // Get user's answer

                     // Determine the status of the question (will always be Wrong here, but keeping structure)
                    String status = 'Wrong';
                    Color statusColor = Color(0xffFDE4E4); // Wrong color
                    ...
                    if (question['chapter'] != null) {
                      final chapterInfo = chapterMap[question['chapter'].toString()];
                      if (chapterInfo != null) {
                    ...
                    if (question['subchapter'] != null) {
                      final subchapterInfo = subchapterMap[question['subchapter']];
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildChoiceText('A', question['a'], correctAnswer, userChosenAnswer),
                            _buildChoiceText('B', question['b'], correctAnswer, userChosenAnswer),
                            _buildChoiceText('C', question['c'], correctAnswer, userChosenAnswer),
                            _buildChoiceText('D', question['d'], correctAnswer, userChosenAnswer),
